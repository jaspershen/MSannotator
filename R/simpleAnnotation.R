simpleAnnotation <-
function(dataA,max.mz.diff=10,num_nodes=2,queryadductlist=c("M+2H","M+H+NH4","M+ACN+2H","M+2ACN+2H","M+H","M+NH4","M+Na","M+ACN+H","M+ACN+Na","M+2ACN+H","2M+H","2M+Na","2M+ACN+H"),mode="pos",outloc,db_name="KEGG")
{
    ...
    if(queryadductlist=="all" & mode=="pos"){
        ...
        adduct_names<-adduct_table$Adduct[(adduct_table$Mode=="positive")]
	
	#adduct_table$Adduct[(adduct_table$Type=="S" & adduct_table$Mode=="positive") | (adduct_table$Type==gradienttype & adduct_table$Mode=="positive")]
        ...
        if(queryadductlist=="all" & mode=="neg"){
            ...
            adduct_names<-adduct_table$Adduct[(adduct_table$Mode=="negative")]
   ...
   # print("dim of Db")
    ...
    clusterExport(cl, "find.Overlapping.mzs")
       ...
       l2<-parLapply(cl,s1,Annotationbychemical_IDschild,dataA=dataA, queryadductlist=c(adduct_names),
       max.mz.diff=max.mz.diff,outloc=outloc,keggCompMZ=chemCompMZ,otherdbs=FALSE,otherinfo=FALSE,adduct_table=adduct_table)
       }else{
       
        l2<-lapply(cl,1,function(k){
		res<-Annotationbychemical_IDschild(k,dataA=dataA, queryadductlist=c(adduct_names),
       max.mz.diff=max.mz.diff,outloc=outloc,keggCompMZ=chemCompMZ,otherdbs=FALSE,otherinfo=FALSE,adduct_table=adduct_table)
		return(res)
	})
       ...
       fname<-paste("mzAnnotation",db_name,".txt",sep="")
       
       write.table(levelB_res,file=fname,sep="\t",row.names=FALSE)

...
if(is.na(adduct_weights)==TRUE){
 data(adduct_weights)
		...
		mchemdata$time<-as.numeric(as.character(mchemdata$time))
			...
			#for(m in  1:length(curmchemdata$mz))
                      ...
                      isp_masses_mz_data<-lapply(1:length(curmchemdata$mz),function(m)
		       {
							...
							return(put_isp_masses_curmz_data)
						       ...
						       # isp_masses_mz_data<-rbind(isp_masses_mz_data,put_isp_masses_curmz_data)
                        })
...
colnames(isp_masses_mz_data)<-cnames
	...
	if(FALSE){
			isp_masses_mz_data<-cbind(isp_masses_mz_data[,c(3:4)],isp_masses_mz_data[,c(1:2)],isp_masses_mz_data[-c(1:4)])
			colnames(isp_masses_mz_data)<-c(cnames[3],cnames[4],cnames[1],cnames[2],cnames[5:6])
			isp_masses_mz_data<-as.data.frame(isp_masses_mz_data)

			print(head(isp_masses_mz_data))
		}
        ...
        level_module_isop_annot=isp_masses_mz_data,adduct_table=adduct_table,adduct_weights=adduct_weights,degree_weights=alldegrees,filter.by,max_isp=max_isp,
	MplusH.abundance.ratio.check=MplusH.abundance.ratio.check,mass_defect_window=mass_defect_window,mass_defect_mode=mass_defect_mode)
        ...
        #cur_chem_score<-(chem_score$degreeweights[,1]+1)*chem_score$chemical_score
...
"chemids_split","dataA","corthresh","max.mz.diff","outloc","num_sets","db_name","num_nodes","num_sets","adduct_weights","alldegrees","filter.by")
}

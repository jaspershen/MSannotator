...
function(dataA,queryadductlist=c("M+H"),adduct_table,max.mz.diff=10,outloc,numnodes=10, otherdbs=FALSE,otherinfo=FALSE,keggCompMZ,syssleep=0.01){
	   ...
	   clusterExport(cl, "getVenn")
		...
		for(mind in seq(1,length(mzlist),mz_group)){
			...
			if(num_mz>100){
				...
				cur.annot.res<-parLapply(cl,s1,Annotationbychemical_IDschild,queryadductlist,adduct_table,max.mz.diff,outloc, otherdbs,otherinfo,keggCompMZ)
				...
				#for(i in 1:length(s1))
				{
					#print("mz is")
					#print(s1[i])
					rescur<-Annotationbychemical_IDschild(s1,queryadductlist,adduct_table,max.mz.diff,outloc, otherdbs,otherinfo,keggCompMZ)
					#print(length(rescur))
					if(length(rescur)>0){
